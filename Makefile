...
release-artifacts: validate-app-version
	CGO_ENABLED=0 GOOS=darwin   GOARCH=amd64 $(GO) build $(LDFLAGS) -o ./OPATH/mockster-$(PROGVER).darwin-amd64       -a -v $(MOCKSTER_MAIN)/*.go && tar cvfz ./OPATH/mockster-$(PROGVER).darwin-amd64.tar.gz  ./OPATH/mockster-$(PROGVER).darwin-amd64
	CGO_ENABLED=0 GOOS=linux    GOARCH=amd64 $(GO) build $(LDFLAGS) -o ./OPATH/mockster-$(PROGVER).linux-amd64        -a -v $(MOCKSTER_MAIN)/*.go && tar cvfz ./OPATH/mockster-$(PROGVER).linux-amd64.tar.gz   ./OPATH/mockster-$(PROGVER).linux-amd64
	CGO_ENABLED=0 GOOS=linux    GOARCH=arm64 $(GO) build $(LDFLAGS) -o ./OPATH/mockster-$(PROGVER).linux-arm64        -a -v $(MOCKSTER_MAIN)/*.go && tar cvfz ./OPATH/mockster-$(PROGVER).linux-arm64.tar.gz   ./OPATH/mockster-$(PROGVER).linux-arm64
	CGO_ENABLED=0 GOOS=windows  GOARCH=amd64 $(GO) build $(LDFLAGS) -o ./OPATH/mockster-$(PROGVER).windows-amd64.exe  -a -v $(MOCKSTER_MAIN)/*.go && tar cvfz ./OPATH/mockster-$(PROGVER).windows-amd64.tar.gz ./OPATH/mockster-$(PROGVER).windows-amd64.exe
